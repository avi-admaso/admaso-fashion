for (var i = 0; i < clothes.length; i++) {
  if (clothes[i].category == "shoes") {
    shoesSection.innerHTML += ` <div id = "shoesDiv"><p class = "shoesInfo"> NAME - ${clothes[i].name}<br> PRICE - ${clothes[i].price}$<br> DESCRIPTION - ${clothes[i].description}</p>
    <button onclick = "addItem(add(${i + 1}))" type="button" id = "${
      clothes[i].id
    }"class="addButton" title="add to cart!">+</button>
    <img onclick="theNextImg(event)" data-secondImg="${clothes[i].photo[1]}" class="shoesImg" src="${clothes[i].photo[0]}" alt=""></div>`;
  }
}
function add(id) {
